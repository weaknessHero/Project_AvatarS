...
CREATE TABLE "user" ("id" bigint NOT NULL auto_increment PRIMARY KEY,"username" varchar_ignorecase(255),"domain" bigint,"desc" varchar_ignorecase(255));
> ok

-- adds constraint on (domain,username) and generates unique index domainusername_key_INDEX_xxx
ALTER TABLE "user" ADD CONSTRAINT "domainusername_key" UNIQUE ("domain","username");
> ok
...
ALTER TABLE "user"  ADD CONSTRAINT "udomain_fkey" FOREIGN KEY ("domain") REFERENCES "domains"("id") ON DELETE RESTRICT;
> ok
...
ALTER TABLE "user" DROP CONSTRAINT "domainusername_key";
> ok

insert into "domains" ("id") VALUES (1);
> update count: 1

insert into "user" ("username","domain","desc") VALUES ('test',1,'first user');
> update count: 1

-- should work,because we dropped domainusername_key, but failed: Unique index or primary key violation
INSERT INTO "user" ("username","domain","desc") VALUES ('test',1,'second user');
> update count: 1
